...
function getAccountCounters(s, classes=[]) {
  ...
  const allDatesRaw=activeCombosForCount.flatMap(c=>{
    ...
    const _pi2=dates.map(d=>{const cl=myClasses.find(x=>x.date===d);return{date:d,paused:!!(cl&&(cl.paused||cl.cancelType==="paused"))};});
    ...
    if(_pC2>0&&_npC2>0&&!_afp2&&!_ae2){
      ...
      const _pc2=myClasses.find(cl=>(cl.occurrences||[]).some(od=>dates.includes(od)));
  ...
  const ClassCard=({c})=>{
    ...
    <WhiteCard style={{marginBottom:12,background:cardBg,border:cardBorder}}>
      ...
      {c.date>=WEEK_AGO&&(()=>{
        ...
        if(isCancelledPure) return <button onClick={()=>onSaveClass({...c,cancelled:false,cancelType:null,rescheduledTo:null,applyToAll:false,_reactivating:true},true)} style={{width:"100%",padding:"10px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#1565C0,#42A5F5)",color:C.white,fontSize:12,cursor:"pointer",fontWeight:700}}>↩ Reactivar clase</button>;
        if(isReprogDone) return <button onClick={()=>onSaveClass({...c,cancelled:false,cancelType:null,rescheduledTo:null,applyToAll:false,_reactivating:true},true)} style={{width:"100%",padding:"10px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#1565C0,#42A5F5)",color:C.white,fontSize:12,cursor:"pointer",fontWeight:700}}>↩ Volver a fecha original</button>;
...
export default function App() {
  ...
  // --- Extracted from handleSaveClass (A4: StudentPacks) ---
