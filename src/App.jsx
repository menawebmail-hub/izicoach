...
function Agenda({ students, classes, rawClasses, onSaveClass, onAttendance, onAddStudent, courts=[], packages=[], onUpdateStudent, onDeleteClass, pendingReprog, onClearPendingReprog, onAddPackage, onRefresh }) {
  ...
  const ClassCard=({c})=>{
    ...
    <WhiteCard style={{marginBottom:12,background:cardBg,border:cardBorder}}>
      ...
      {c.date>=TODAY_DATE&&<button onClick={()=>{if(c.cancelled){onSaveClass({...c,cancelled:false,cancelType:null,rescheduledTo:null,applyToAll:false},true);}else{setShowCancel(c);}}} style={{width:"100%",padding:"10px",borderRadius:10,border:"none",background:c.cancelled?"linear-gradient(135deg,#1565C0,#42A5F5)":"linear-gradient(135deg,#E65100,#FF8F00)",color:C.white,fontSize:12,cursor:"pointer",fontWeight:700}}>{c.cancelled?"↩ Reactivar clase":"📅 Reprogramar / Cancelar"}</button>}
    </WhiteCard>
    );
  };
