  ...
  const ClassCard=({c})=>{
    ...
    <WhiteCard style={{marginBottom:12,background:cardBg,border:cardBorder}}>
      ...
      {c.date>=TODAY_DATE&&(()=>{
        ...
        if(isCancelledPure||isReprogDone) return <button onClick={()=>onSaveClass({...c,cancelled:false,cancelType:null,rescheduledTo:null,applyToAll:false},true)} style={{width:"100%",padding:"10px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#1565C0,#42A5F5)",color:C.white,fontSize:12,cursor:"pointer",fontWeight:700}}>↩ Reactivar clase</button>;
