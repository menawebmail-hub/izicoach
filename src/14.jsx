...
export default function App() {
  ...
  const setUserWithRef=(u)=>{setUser(u);userRef.current=u;};
  ...
  const setStudents=(v)=>{const next=typeof v==="function"?v(students):v;setStudentsRaw(next);lsSet("izi_students",next);if(userRef.current)syncToSupabase("students",next,userRef.current.id);};
  const setClasses=(v)=>{const next=typeof v==="function"?v(classes):v;setClassesRaw(next);lsSet("izi_classes",next);if(userRef.current)syncToSupabase("classes",next,userRef.current.id);};
  ...
  const setCoachProfile=(v)=>{const next=typeof v==="function"?v(coachProfile):v;setCoachProfileRaw(next);lsSet("izi_profile",next);if(userRef.current)supabase.from("coaches").upsert({id:userRef.current.id,...next}).then(()=>{});};
  const setExpenses=(v)=>{const next=typeof v==="function"?v(expenses):v;setExpensesRaw(next);lsSet("izi_expenses",next);if(userRef.current)syncToSupabase("expenses",next,userRef.current.id);};
